...
const jackInlineStyles = (e) => {
  ...
  // handle copy-only override of text-align
  // blanket override flavor to be start-aligned
  if (e.classList.contains("copy-align-start") || e.classList.contains("flavor")) {
    otherStyles.push("text-align: start;")
    reverses.push({
      element: e,
      textAlign: styles.getPropertyValue("text-align"),
    });
  } else {
    otherStyles.push(`text-align: ${styles.getPropertyValue("text-align")};`);
  }
...
const copyContents = async (btnElt, btnText) => {
  ...
  if (copyable) {
    ...
    // undo copy-only style changes
    reverses.forEach(r => {
      r.element.style.textAlign = r.textAlign;
    })
  }
  if (window.getSelection()) window.getSelection().removeAllRanges();
